...
- **自修复配置** — `opencli doctor` 诊断 bridge 连通性与扩展状态
...
OpenCLI 统一通过 Browser Bridge 架构与浏览器通信。对浏览器命令，文档统一采用一套显式配置模型：扩展侧配置 `backendUrl` + `token`，CLI 侧传 `--remote-url` + `--token` + `--client`。
...
完成！后续执行浏览器命令时，统一按 Browser Bridge 方式配置扩展和 CLI 参数。
...
- 所有 Browser Bridge 浏览器命令
- CLI 和浏览器扩展不在同一台机器
- 你有多个在线浏览器客户端，需要显式选择其中一个

CLI 侧必填参数：

- `--remote-url` 或 `OPENCLI_REMOTE_URL`
- `--token` 或 `OPENCLI_REMOTE_TOKEN`
- `--client` 或 `OPENCLI_REMOTE_CLIENT`
